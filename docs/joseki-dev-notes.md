...
**数据来源**：weiqi-joseki 技能包的 database.json，约 10 万条定式。
...
**问题**：原动态分桶方案在点击第二着时需加载 191 个分桶，性能差。

**新方案**：
```
trie-index.json.gz    → 索引 trie（约 20KB）
trie-pd.json.gz       → pd 前缀子树（约 25KB）
trie-pc.json.gz       → pc 前缀子树（约 16KB）
...约 57 个子树文件
```

**裁剪策略**：
- 后序遍历 trie 树
- 统计节点的定式数（有 leaf 标记的节点）
- 当定式数 >= 阈值（1000）时，裁剪为子树文件
- 子树文件内部递归裁剪
...
**前端加载流程**：
1. 加载 trie-index.json.gz（索引）
2. 在索引中遍历首着
3. 用户点击首着后，检查是否是裁剪点（有 subtree 指针）
4. 如果是裁剪点，加载对应子树文件
5. 合并子树到当前节点（node.children = subtree.children）
6. 继续遍历
